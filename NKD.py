...
class SimpleSeq2Seq(Model):
    ...
    def __init__(
        ...
        self._decoder_input_dim = self.hidden_dim*2+total_entiy
        ...
        # with open('data_0108/sym_sym_trainandval_01.pk', 'rb') as f:
        #     self.symp_mat = torch.tensor(pickle.load(f)).float().cuda()

        # with open('data_0108/sym_list.pk','rb') as f:
        #     self.idx_word = pickle.load(f)
        # self.vocab_to_idx = {}
        # self.idx_to_vocab_list = []
        # for k, word in self.idx_word.items():
        ...
        self.entity_linear = Linear(self.hidden_dim*2, 1)
    ...
    def forward(self, tags, history, next_sym, source_tokens, his_symptoms, target_tokens, **args):
        ...
        encoder_outputs = self._encoder(embedded_input, source_mask) # bs * dim
        kg_encoder_output = self._kg_encoder(embedded_input, source_mask)  # bs * dim
        ...
        entity_weight = torch.sigmoid(self.entity_linear(entity_feature))  # bs * sym_size
        # print("entity_weight: ", entity_weight.size())
        topic_weight = torch.ones_like(next_sym) + 4 * next_sym
        # print("entity_weight: ", entity_weight.size())
        # print("next_symp: ", next_sym.size())
        entity_loss = torch.nn.functional.binary_cross_entropy(entity_weight.squeeze(-1), next_sym.float(), weight=topic_weight.float())
        # if self.training:
        ...
        #         with open('save_fd/human_eval/pretrain/NKD_pret_dis9_test1.txt', 'a+', encoding='utf-8') as f:
