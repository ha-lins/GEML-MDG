...
#         response_tokens = response_tokens[-self.max_tokens:]
...
#         # fields["topics"] = MultiLabelField(list(sample['next_symp']), skip_indexing=True, num_labels=77)
#         fields['target_tokens'] = TextField(response_tokens, self._target_token_indexers)
#         # fields['symptoms'] = MultiLabelField(list(sample['his_symp']), skip_indexing=True, num_labels=77)
#         fields['tags'] = MetadataField(sample['tags'][-sen_num:])
#         fields['history'] = ListField(fileds_list)
#         # fields['future'] = MultiLabelField(list(sample['future_symp']), skip_indexing=True, num_labels=77)
...
class KD_Metric(Metric):
    def __init__(self) -> None:
        ...
        self.pred_diease = {}
        self.true_diease = {}
        with open('cy/cy_bio_sym.pk', 'rb') as f:
    ...
    def convert_sen_to_entity_set(self, sen):  # 如果只统计knowledge的话，那么恶心呕吐占了大便宜，得统计正确的knowledge，也就是正确的topic
        entity_set = set()
    ...
    def __call__(
        ...
        dis_list = [2, 6, 48, 9, 40, 27, 46, 35, 33, 18, 13, 42, 73, 30, 52]
        ...
        for batch_num in range(len(references)):
            ...
            d_r, d_h = [], []
            # for r in hypo_list:
            #     if r in self.dis_list< 6:
            #         self.pred_diease[dialog_index[batch_num]] = r
            #         # print('[info] r is:{}'.format(r))
            #
            # for h in ref_list:
            #     # print('[info] ref_list is:{}'.format(ref_list))
            #     if h in self.dis_list< 6:
            #         self.true_diease[dialog_index[batch_num]] = h
            #         # print('[info] h is:{}'.format(h))
...
class Distinct1(Metric):
    ...
    def get_metric(self, reset: bool = False):
        value = len(self.appear_vocabs) / (self._total_vocabs + 1e-9)
        if reset:
            self.reset()
        return value
