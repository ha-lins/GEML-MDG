...
class MetaTrainer(Trainer):
    def __init__(self,
                 ...
                 # rank: int = 0,
                 # world_size: int = 1,
                 cuda_device: Union[int, List] = -1, #int = -1,
        ...
        super().__init__(model, optimizer, iterator, train_datasets, cuda_device, **kwargs)
    ...
    def batch_loss(self, batch_group: List[TensorDict], for_training: bool) -> torch.Tensor:
        ...
        if len(self.cuda_device) > 1:
            print('[info] multiple gpu is:{}'.format(self.cuda_device))
            output_dict = training_util.data_parallel(batch_group, self.model, self.cuda_device)
        else:
            # print("==============The length of batch_group is ")
            print(len(batch_group))
            print(batch_group)
            assert len(batch_group) == 1
            batch = nn_util.move_to_device(batch_group, self.cuda_device[0])
    ...
    def reptile_outer_update(self, train_generators: List[List[Iterable]], iteration: int, num_gpus: int):
        ...
        task_wrap = Tqdm.tqdm(zip(train_generators[0], train_generators[1], train_generators[2], train_generators[3], train_generators[4]), \
                              total=1)
        # for _ in range(self.meta_batch_size):
        for i, batch_group in enumerate(task_wrap):
            for k in range(self.meta_batch_size):  # tasks per batch
                total_loss += self.reptile_inner_update(batch_group[k])
        ...
        outerstepsize = self.meta_step_size * (1 - iteration / self.meta_batches) # linear schedule
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for i in range(0, self.meta_batches):
            train_generators = []
            # print('=====train_data is:{}'.format(len(self.train_data), len(self.train_data[0])))
            for i, train_info in enumerate(self.train_data):
                # print('=====train_data is:{}'.format(train_info))
                # print('=====iterator is:{}'.format(self.iterator))
                raw_train_generator = self.iterator(train_info,
                                                    num_epochs=1,
                                                    shuffle=self.shuffle)
                train_generators.append(lazy_groups_of(raw_train_generator, num_gpus))

            loss_batch = self.reptile_outer_update(train_generators, i, num_gpus)
