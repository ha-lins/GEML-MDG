...
class MetaTrainer(Trainer):
    ...
    def batch_loss(self, batch: TensorDict, for_training: bool) -> torch.Tensor:
        ...
        if self._multiple_gpu:#len(self.cuda_device) > 1:
            ...
            batch = nn_util.move_to_device(batch, self.cuda_device[0])
    ...
    def reptile_outer_update(self, train_generators: List[Iterable], iteration: int, num_gpus: int):
    ...
    def from_params(cls,  # type: ignore
        ...
        if model_device >= 0:
            print('[info]model device is:{}'.format(model_device))
